...
public class VoronoiGenerator implements BasicGraphGenerator
{
	...
	@Override
	public SimpleGraph<Node, Border> generate(int num ,Random rand) 
	{
		int seed = rand.nextInt(Integer.MAX_VALUE);
		if(!generatorIsSetup)
		{
			System.err.println("The generator is not setup, default values will be used.");
			numPoints = num;
			drawMap = true;
			generatorIsSetup = true;
		}

		SimpleGraph<Node, Border> graph =  generateGraph(seed);
		ConnectivityInspector<Node, Border> inspector = new ConnectivityInspector<>(graph);
		...
		{
			return generate(num,rand);
		}
	}
	
	 Color oceanColor = color(68, 68, 122);
	 Color defaultOceanColor = color(68, 68, 122);
	 int numPoints;
	 protected int width = 600;
	 protected int height = 600;
	 boolean drawBorders = false;
	 boolean createIsland = false, noisyEdges = false;
	 boolean showText = false;
	 //int seed;
	 boolean drawMap = false;
	 boolean drawnOnce = false;
	 protected static VoronoiGenerator instance;

	 VoronoiGeneratorDrawer currentDrawer;
	
	 boolean generatorIsSetup = false;
	public void setupGenerator(int numPoints, boolean drawMap, boolean drawBorders,
			int width, int height,boolean createIsland,boolean noisyEdges,boolean showText) 
	{
		...
		this.noisyEdges = noisyEdges;
		this.showText = showText;		
		this.drawMap = drawMap;	
		if(!drawMap && createIsland)
		{
			System.err.println("Warning: Islands are not supported in offline generation. Normal Voronoi diagram will be created instead.");
		}
		this.generatorIsSetup = true;
	}
	
	private  SimpleGraph<Node, Border> generateGraph(int seed)
	{
	...
	ArrayList<PVector> buildNoisyLineSegments(PApplet ap,PVector A, PVector B, PVector C, PVector D, int minLength)
	{
	...
	private static Color color(double c) {
...
/************** CLASSES ******************/
	...
	class TriangleVectorComparator implements Comparator<PVector> {
		private PVector M;
	...
	class Voronoi {
	 ...
	 class PoissonDistribution {
		...
		private float alternateRandomRange(float min , float max)
		{			
	 ...
	 class Map {
		...
		Color color(double r, double g, double b) {
		 ...
		 Color[] biomesColor = { color(0, 0, 0), // void
				oceanColor, // Ocean
		...
		public  class Face {
		 ...
		 void drawPathForwards(PApplet ap, ArrayList<PVector> pts) {
			for (PVector p : pts)
				ap.vertex(p.x, p.y);
		}

		 void drawPathBackwards(PApplet ap, ArrayList<PVector> pts) {
